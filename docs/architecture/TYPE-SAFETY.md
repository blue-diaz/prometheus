...
# [BLOQUEADO] Sistema de Type Safety do Prometheus
...
## [BUSCA] Tipos de Detecção

### 1. Tipo Inseguro: `any`

**Padrão:** PROTECTED_1 `: any`

**Categoria:** `tipo-inseguro-any`

**Contextos** Detectados: PROTECTED_1
...
"[IDEIA] Analise uso da variável e defina tipo específico ou use unknown com type guards";

// Catch block
"'any' em catch block 'error' - TypeScript recomenda 'unknown'";
"[IDEIA] Substitua por: catch (error: unknown) { ... }";
```

### 2. Type Assertions: `as any`

**Padrão:** PROTECTED_1 `as any`

**Categoria:** `tipo-inseguro-any-assertion`

**Nível:** PROTECTED_1 `erro` (mais severo que declaração)
...
"[IDEIA] Substitua por tipo específico ou use unknown com validação runtime"
...
// [IDEIA] import type { Node } from "@babel/types"

// Callbacks genéricos (70%)
function onChange(opts: unknown) {}
// [IDEIA] Definir interface específica: OnChangeOptions

// Filter/Map (75%)
const filtered = items.filter((item: unknown) => {});
// [IDEIA] Tipar array pai: items: Item[]

// Relatórios (70%)
interface Relatorio {
  data: unknown; // Melhorável
}
// [IDEIA] Criar interface específica: RelatorioData
```
...
interface GuardianResult {
  guardian: unknown; // [ERRO] Estrutura conhecida
}
...
## [GRAFICO] Métricas e Estatísticas
...
- `tipo-inseguro-unknown`: 7 ocorrências (53% de redução [BRILHO])
...
## [BIBLIOTECA] API e Extensibilidade
...
**Problema:** PROTECTED_1 Detector reporta casos legítimos como problemas
...
**Problema:** PROTECTED_1 Auto-fix não consegue inferir tipo
...
**Problema:** PROTECTED_1 Análise de tipos está lenta
...
## [LIVRO] Referências e Leituras Adicionais
...
## [ALVO] Roadmap e Melhorias Futuras
...
**Última** atualização: PROTECTED_1 29 de novembro de 2025
**Versão** do documento: PROTECTED_2 1.0.0
**Autor:** PROTECTED_1 Equipe Prometheus
